...
class TravelDecisions:
    ...
    def add_variables(self, m, x, z, tt, w_tour, w_subtour):
        """
        ...
        if len(self.modes) > 1 or len(self.time_periods) > 1 or any([len(a.locations) > 1 for a in self.activities]):
            ...
            self._add_mode_specific_travel_time_constraint(m, tt, prod, z, mode_choice, location_choice,
                                                           time_slot_choice)
        else:
    ...
    def _add_mode_choice_variable(self, m):
        ...
        mode_ch = {(a, mode): m.addVar(vtype=GRB.BINARY, name=f'mode_ch_{a}_{mode}', lb=0, ub=1)
                   for a in self.act_labels for mode in self.modes}

        # one mode per trip only
        for a in self.act_labels:
            m.addConstr(gp.quicksum(mode_ch[a, mode] for mode in self.modes) == 1)
    ...
    def _subtour_based_mode_choice_constraints(self, m, z, mode_ch, w_tour, w_subtour):
        ...
        two_prim_acts_in_tour = {a: m.addVar(vtype=GRB.BINARY, name=f'two_prim_act_in_tour_{a}')
        ...
        for a in self.act_set.get_acts_wo_home():
            m.addConstr(two_prim_acts_in_tour[a.label] == gp.quicksum((a.is_primary * two_prim_act_tours.get(t, 0) *
        ...
        for a in self.act_labels_wo_dusk:
            for b in self.act_labels_wo_home:
                for mode in self.modes:
                    ...
                    # place in the tour
                    m.addConstr(z[a, b] <= mode_ch[a, mode] - mode_ch[b, mode] + 1 + two_prim_acts_in_tour[b])
                    m.addConstr(z[a, b] <= mode_ch[b, mode] - mode_ch[a, mode] + 1 + two_prim_acts_in_tour[b])
            ...
            for p in [p.label for p in self.activities if p.is_primary]:
                for q in [q.label for q in self.activities if (q.is_primary and (p != q.label))]:
                    for mode in self.modes:
                        ...
                        m.addConstr(mode_ch[a, mode] >= mode_ch[p, mode] - 1 + z[a, q])
                        m.addConstr(mode_ch[a, mode] <= mode_ch[q, mode] + 1 - z[a, p])
        ...
        for a in self.act_labels:
            for mode in [m for m in self.home_based_modes]:
                for p in [p.label for p in self.activities if p.is_primary]:
                    ...
                    m.addConstr(mode_ch[p, mode] <= mode_ch[a, mode] + 1 - z[a, p] + w_subtour[a])

    def _add_location_choice_variable(self, m):
        # location choice variable
        loc_choice = {(a.label, l.name): m.addVar(vtype=GRB.BINARY, name=f"loc_choice_{a.label}_{l.name}")
        ...
        for a in self.activities:
            m.addConstr(gp.quicksum(loc_choice[a.label, l.name] for l in a.locations) == 1)
            if a.location_group:
        ...
        for acts in location_groups.values():
            ...
            for a in acts[1:]:
                for l in first_act.locations:
                    m.addConstr(loc_choice[first_act.label, l.name] - loc_choice[a.label, l.name] == 0)

        return loc_choice

    def _add_time_slot_variable(self, m, x):
        # time period choice variable
        time_slot_ch = {(a, time_slot): m.addVar(vtype=GRB.BINARY, name=f'time_slot_{a}_{time_slot}')
        ...
        for a in self.act_labels:
            ...
            m.addConstr(gp.quicksum(time_slot_ch[a, time_slot] for time_slot in self.time_periods.keys()) == 1)

            for time_slot, interval in self.time_periods.items():
                # force time_slot_ch[a, i] = 0 when a's endtime is out of its bounds
                m.addConstr(x[a] >= interval[0] - big_m * (1 - time_slot_ch[a, time_slot]))
                m.addConstr(x[a] <= interval[1] + big_m * (1 - time_slot_ch[a, time_slot]))
    ...
    def _add_location_mode_time_slot_product(self, m, z, mode_ch, loc_ch, time_period_ch):
        ...
        mode_loc_time_decision_prod = {
            (a.label, b.label, o.name, d.name, mode, time_slot):
                m.addVar(vtype=GRB.BINARY, name=f'mode_ch_loc_ch_product_{a}_{b}_{o}_{d}_{mode}_{time_slot}')
                    ...
                    for _d in _b.locations:
                        ...
                        for mode in self.modes:
                            for tp in self.time_periods.keys():
                                m.addConstr(mode_loc_time_decision_prod[a, b, o, d, mode, tp] <= z[a, b])
                                m.addConstr(mode_loc_time_decision_prod[a, b, o, d, mode, tp] <= mode_ch[a, mode])
                                m.addConstr(mode_loc_time_decision_prod[a, b, o, d, mode, tp] <= loc_ch[a, o])
                                m.addConstr(mode_loc_time_decision_prod[a, b, o, d, mode, tp] <= loc_ch[b, d])
                                m.addConstr(mode_loc_time_decision_prod[a, b, o, d, mode, tp] <= time_period_ch[a, tp])
                                m.addConstr(mode_loc_time_decision_prod[a, b, o, d, mode, tp] >= z[a, b] +
                                            mode_ch[a, mode] + loc_ch[a, o] + loc_ch[b, d] + time_period_ch[a, tp] - 4)

        return mode_loc_time_decision_prod

    def _add_mode_specific_travel_time_constraint(self, m, tt, mode_loc_time_decision_prod, z, mode_ch, loc_ch,
                                                  time_period_ch):
        # travel time calculation. not an actual decision, more a look-up from an external matrix
        for a in self.activities:
            m.addConstr(tt[a.label] == gp.quicksum(self.travel_dict.get_value(m, 'travel_times', tp, o, d) *
                                                   mode_loc_time_decision_prod[
                                                       a.label, b.label, o.name, d.name, m, tp]
                                                   for b in self.activities for o in a.locations for d in
                                                   b.locations
    ...
    def _add_simple_travel_time_constraint(self, m, tt, z):
        ...
        for a in self.activities:
            m.addConstr(
                tt[a.label] == gp.quicksum(z[a.label, b.label] * self.travel_dict.get_value(m, 'travel_times', tp, o, d)
    ...
    def _add_travel_cost(self, m, tt, travel_utilities):
        ...
        travel_cost = {a: m.addVar(vtype=GRB.CONTINUOUS, name=f'travel_cost_{a}', lb=-inf, ub=inf) for a in
                       self.act_labels}
        for a in self.act_labels:
            ...
            m.addConstr(travel_cost[a] == travel_utilities['p_travel'] * tt[a])
        return travel_cost
