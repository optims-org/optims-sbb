...
class ActivitySequence:
    ...
    def add_restrictions(self, m: Solver, w, z):
        """
    ...
    def _fix_activity_participation(self, m, w):
        ...
        for a in self.activities:
            ...
            m.Add(w[a.label] >= a.participation)  # enforce activities to take place if participation is fixed
        ...
        for a in act_labels_wo_dusk:
            ...
            for b in [b.label for b in self.activities if b.act_type not in [HOME_NAME, DAWN_NAME, DUSK_NAME]]:
                for tour_no in self.tour_numbers:
                    ...
                    m.Add(z[a, b] <= w_tour[a, tour_no] - w_tour[b, tour_no] + 1)
                    m.Add(z[a, b] <= w_tour[b, tour_no] - w_tour[a, tour_no] + 1)
    ...
    def _fix_number_of_primary_activities_per_tour(self, m, w_tour):
        ...
        for t in self.tour_numbers:
            ...
            m.Add(m.Sum(w_tour[a.label, t] for a in self.activities if a.is_primary) == prim_acts_tour_no.count(t))
    ...
    def _fix_tour_types(self, m, w, w_tour):
        ...
        for a in [a for a in self.activities if a.act_type not in [HOME_NAME, DAWN_NAME, DUSK_NAME]]:
            m.Add(m.Sum(w_tour[a.label, t] for t, t_type in tour_dict.items() if t_type == a.tour_type) >= w[a.label])
    ...
    def _add_subtour_indicator(self, m, z):
        ...
        w_subtour = {a.label: m.IntVar(name=f'w_subtour_{a}', lb=0, ub=1) for a in self.activities}
        ...
        # activities.
        m.Add(len(two_prim_act_tours) == m.Sum(w_subtour[a.label] for a in self.activities if a.is_primary))
        ...
        for a in [a.label for a in self.activities]:
            for b in [b.label for b in self.activities if not b.is_primary]:
                ...
                m.Add(z[a, b] <= w_subtour[a] - w_subtour[b] + 1)
                m.Add(z[a, b] <= w_subtour[b] - w_subtour[a] + 1)
    ...
    def _fix_subtour_activities(self, m, w_subtour):
        ...
        for a in self.activities:
            if a.act_type in [HOME_NAME, DAWN_NAME, DUSK_NAME]:
                ...
                m.Add(w_subtour[a.label] == 0)
            elif a.is_subtour and not a.is_primary:
                m.Add(w_subtour[a.label] == 1)
